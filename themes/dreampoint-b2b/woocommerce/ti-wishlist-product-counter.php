...
$text_attr = trim( wp_strip_all_tags( $text_plain ) );

if ( $text_attr === '' ) {
    $text_attr = apply_filters(
        'tinvwl_default_wishlist_title',
        tinv_get_option( 'general', 'default_title' )
    );

    if ( empty( $text_attr ) ) {
        $text_attr = __( 'Lista želja', 'dreampoint-b2b' );
    }
}
?>
