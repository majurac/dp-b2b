...
**Ažuriranje 2026-09-21 (TEST Apros pristup + read-only inspekcija, `docs/decisions.md` ADR-008):** BL-01 (rang 1) je efektivno RESOLVED — TEST pristup postoji. AP-06 (rang 2, idempotency) je DJELOMIČNO RESOLVED — payload/response oblik i idempotency mehanizam potvrđeni read-only inspekcijom. AP-07 (rang 5) je DJELOMIČNO RESOLVED — šema dostavnih lokacija (`recipient_code`, više lokacija po partneru) potvrđena. DP-01 (rang 4) je suštinski RESOLVED na nivou šeme — trenutni model (1 WP user : 1 partner_code : N delivery lokacija) prirodno podržava oba klijentska scenarija naloga. PL-01 (rang 7) i WH-01 (rang 8) OSTAJU otvoreni — inspekcija je pokazala da se partner sync trenutno radi ručnim CSV importom (ne live Apros endpoint), a warehouse stock se mapira kao jedan flattened broj bez per-warehouse strukture. DP-02/BL-06 (rang 6) — provenance potvrđena kao stvarna (nije stale), djelomično rasvijetljena kroz warehouse-splitting odgovor (vidi ADR-008), ali specifično "Josip" mapiranje ostaje formalno nepotvrđeno.
...
**Stanje (ažurirano 2026-09-22 — zvanična ZGData API dokumentacija primljena):** Payload primjer koji je nedostajao od 2026-07-02 sada je dostavljen — zvanična dokumentacija (`API DOKUMENTACIJA — Dreampoint - B2B integracija`, v1.0, ZGData 2026) definiše egzaktnu šemu i primjer odgovora za `partnerBrandDiscountList/get` (`partnerCode`, `name`, `vatId`, `email`, `brandId`, `brandName`, `discountPercent`) i `countryPriceList/get` (`articleId`, `countryPriceListCode`, `price`). Ovo razrješava specifično traženi payload-primjer gap. **Ne razrješava** cjelokupno pitanje krajnje pricing prioritizacije van onoga što je već arhitekturalno potvrđeno 2026-07-02 — dokument je čista API referenca, ne poslovno pravilo.
...
**Stanje (2026-09-22 — zvanična ZGData API dokumentacija primljena):** Bez promjene. Primljeni dokument (`API DOKUMENTACIJA — Dreampoint - B2B integracija`, v1.0, ZGData 2026) pokriva isključivo katalog/partner/pricing GET endpointe — **ne dokumentuje** `order/create`, outbound order payload, `partnerDeliveryLocationId` unutar tog payload-a, response format, idempotency niti shipping-address precedence. AP-06 ostaje u potpunosti otvoren na istom nivou kao 2026-07-02.
...
**Stanje (2026-09-22 — zvanična ZGData API dokumentacija primljena):** Payload primjer koji je nedostajao od 2026-07-02 sada je zvanično dostavljen — `partnerDeliveryLocationList/get` šema: `partnerCode`, `recipientCode` (zvanično opisan kao "šifra lokacije/primatelja dostave"), `name`, `adress`, `city`, `postalCode`, `recipientEmail`. Ovo dodatno potvrđuje provenance postojećeg lokalnog mapiranja (`recipient_code` u `apros_delivery_locations` tabeli, već implementirano i staging-validirano — vidi ispod). **I dalje nedostaje:** stabilnost location ID-a između sync ciklusa — dokument to ne adresira.

**Checkout UI status (ažurirano 2026-09-22):** ✅ **IMPLEMENTIRANO, DEPLOYOVANO I LIVE E2E POTVRĐENO NA APROS SANDBOX-U** — hibridna 0/1/2+ arhitektura (`inc/checkout-delivery-location.php`, commit `8577565`) je na stagingu (HEAD potvrđen). 2+ grana **validirana na realnim staging Apros podacima** (partner_code 2870, 8 lokacija): selektor se renderuje, prazno initial stanje, nula ERP terminologije, native required-validacija blokira submit, izbor se prihvata, no-partner-code regresija prolazi. Nakon eksterne Apros potvrde sandbox statusa (2026-09-22, isti dan), **finalna `_apros_delivery_location_id` persistencija i new-order empty-selection reset su live E2E potvrđeni preko stvarno kompletirane i Apros-om prihvaćene sandbox narudžbe (#23358, ERP broj 4244)**. 0-lokacija i 1-lokacija grane ostaju code-review + izolovana lokalna simulacija (nema prirodnih 0/1-lokacija partnera na stagingu). Woo Blocks browser-side `localStorage` restoration nijansa — SATISFIED BY DESIGN, sada **LIVE E2E CONFIRMED** (ranija verzija ovog reda je pogrešno navodila "status DEFERRED", pa zatim "live E2E potvrda pending" — oba ispravljena). Puna arhitektura + Staging Acceptance + Apros Sandbox Live E2E detalji: `docs/decisions.md` ADR-010.
...
**DP-B06 — Rezervacija zaliha na razini košarice** ✅ POSLOVNA ODLUKA ZATVORENA (2026-09-21) — tehnička implementacija OTVORENA

**Status:** Klijent je putem `B2B odgovori na pitanja.docx` eksplicitno potvrdio: 1-satna cart-level rezervacija je MANDATORY poslovni zahtjev, klijent prihvaća trošak eventualnog plaćenog plugina, korisnik mora vidjeti vlastiti countdown. Preporuka tima ispod ("zadrži native WC default") je **SUPERSEDED** — zadržana radi istorijskog konteksta, više nije važeća preporuka.

~~**Preporuka tima:** Zadržati nativno WooCommerce ponašanje — rezervacija tek pri kreiranju narudžbe (first-completed-order-wins), bez custom mehanizma.~~ *(superseded 2026-09-21 — vidi `docs/decisions.md` ADR-007)*
...
| ID | Bloker | Rizik |
|----|--------|-------|
| BL-01 | Apros API pristup (sandbox) ne postoji | ✅ EFEKTIVNO RESOLVED (2026-09-21) — TEST Apros ERP pristup uspostavljen; `apros-pricing`, `uncle-dev-importer`, `b2b-partner-importer` aktivni na stagingu. Vidi `docs/decisions.md` ADR-008. Produkcijski pristup nije potvrđen. |
| BL-04 | Order endpoint — puni format payload-a/response-a nepoznat | 🟢 DJELOMIČNO RESOLVED (2026-09-21) — puni payload oblik i idempotency mehanizam potvrđeni read-only inspekcijom (`uncle-dev-importer/order.php`). Pismena Apros potvrda i dalje nedostaje. Vidi ADR-008. |
...
5. ~~**Split shipment logika** — nije definirano ko (WooCommerce ili Apros) razdvaja narudžbe; obje strane moraju biti usklađene~~ **CLOSED 2026-09-21** — klijent potvrdio: Apros interno dijeli po home warehouse-u, webshop šalje jednu konsolidovanu narudžbu, ne obavlja vlastiti split. Read-only inspekcija (`docs/decisions.md` ADR-008) potvrđuje: `order.php` prima `{numberErp, warehouseId}` niz dokumenata od Apros-a — mehanizam postoji i već je djelomično hendlan u kodu.
