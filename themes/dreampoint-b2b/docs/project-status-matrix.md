...
**Checkout UI status (ažurirano 2026-09-22):** ✅ **IMPLEMENTIRANO I DEPLOYOVANO** — hibridna 0/1/2+ arhitektura (`inc/checkout-delivery-location.php`, commit `8577565`) je na stagingu (HEAD potvrđen). 2+ grana **validirana na realnim staging Apros podacima** (partner_code 2870, 8 lokacija): selektor se renderuje, prazno initial stanje, nula ERP terminologije, native required-validacija blokira submit, izbor se prihvata, no-partner-code regresija prolazi. 0-lokacija i 1-lokacija grane te finalna `_apros_delivery_location_id` persistencija u stvarno kompletiranoj narudžbi ostaju code-review + izolovana lokalna simulacija (nema prirodnih 0/1-lokacija partnera na stagingu; kompletiranje narudžbe bi okinulo ERP sync, namjerno izbjegnuto). Woo Blocks browser-side `localStorage` restoration nijansa (otkrivena istim testom) je naknadno razriješena zasebnom WC 11.1.1 source-tracing istragom (2026-09-22) — SATISFIED BY DESIGN, live E2E potvrda i dalje pending — vidi `docs/decisions.md` ADR-010 (ispravljeno ovdje radi konzistentnosti, ranija verzija ovog reda je pogrešno navodila "status DEFERRED"). Puna arhitektura + Staging Acceptance detalji: `docs/decisions.md` ADR-010.
